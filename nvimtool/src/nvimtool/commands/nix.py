...
from ..nix_helpers import build_flake_source, build_plugins_derivation_source
...
def write_plugins(cfg: Config) -> None:
    print("Writing plugin-set.nix")
    nix_data = PluginSpecs.read_json_file(cfg.paths.plugins_declaration)

    flake_nix = build_plugins_derivation_source(nix_data)
    cfg.paths.plugin_set_nix.write_text(flake_nix)
