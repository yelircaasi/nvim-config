...
import argparse
from typing import Callable, Final, Iterable, Literal, NotRequired, Self, TypedDict
...
class Utils:
    ...
    def resolve_path(envvar: str, fallback: str) -> Path:
        ...
        path: Path = from_var or (Path.home() / fallback)
        path.mkdir(exist_ok=True)
        return path

    @staticmethod
    def write_json(obj: list | dict, path: Path) -> None:
    ...
    @staticmethod
    def read_jsonc(path: Path) -> list | dict:
        ...
        return json.loads("".join(map(_strip_comments, path.read_text().splitlines())))
    ...
    @staticmethod
    def run(commands: list[str], **kwargs) -> subprocess.CompletedProcess[bytes]:
        return subprocess.run(commands, **kwargs, check=True, capture_output=True)

    @staticmethod
    def run_optional(commands: list[str], **kwargs) -> subprocess.CompletedProcess[bytes]:
    ...
    @classmethod
    def get_executable_and_version(cls, name: str, subcommand: str | None = None) -> str:
    ...
    @classmethod
    def read_config(cls, path: Path)  -> NvimtoolConfig:
        if not path.exists():
            return {}
        d = cls.read_jsonc(path)
    ...
    def write_table(
        ...
        *,
        ...
        indenter = "\t" * indent
        joiner = f",\n{indenter}"
        body = joiner.join(map(_make_line, d.items()))
        return head + "{\n" + indenter + body + f",\n{indenter[:-1]}}}" + foot
...
type PluginSpecData = list[PluginSpecDict]  # type: ignore
type PluginLockTable = dict[str, PluginLockData | None]  # type: ignore
type ExternalToolSpecData = list[ExternalToolSpec]  # type: ignore
...
class Specs:
    ...
    def from_paths(cls, paths: Paths) -> Self:
        dicts = Utils.read_jsonc(paths.plugins_jsonc)
        return cls(
            _specs=dict(
                (s.name, s)
                for s
                in map(Spec.from_dict, dicts)
            ),
...
class LockData:
    ...
    def from_paths(cls, paths: Paths) -> Self:
        return cls(
            _lock=Utils.read_json(paths.plugins_lock),
            directory=paths.plugin_dir,
        )
    ...
    def get_last_check(self, name: str) -> str:
        return self._lock[name]["last_update"]



def write_plugin_paths_tl(paths: Paths, plugin_lock: PluginLockTable) -> None:
    pd = paths.plugin_dir
    path_dict = {pn: pd / pn for pn in plugin_lock}
...
def write_plugin_layers_tl(paths: Paths, plugin_data: PluginSpecData) -> None:
    layers = {d["layer"]: {} for d in plugin_data}
    for d in plugin_data:
        layer, sublayer = d["layer"], d["sublayer"]
        if not sublayer in layers[layer]:
...
def write_external_tools_tl(
    paths: Paths, external_tool_data: ExternalToolSpecData
) -> None:
...
def transpile_tl(paths: Paths) -> None:
    tl_root, source, target, copy_target = (
        str(paths.tl_dir),
        str(paths.tl_src_dir),
        str(paths.tl_build_dir),
    ...
    cyan_command = [
        ...
        "-b",
        paths.tl_build_dir.relative_to(tl_root).name,
        "build",
        "--prune",
    ]
    print(" ".join(cyan_command))
    Utils.run(cyan_command, cwd=tl_root)
    Utils.run_optional(["stylua", paths.tl_build_dir / "init.lua", paths.tl_build_dir / "lua"])
    Utils.run(["lua", str(paths.cleanup_lua), target])
...
def install_new(paths: Paths) -> None:
    ...
    Utils.write_json(lock, paths.lockfile)
...
def check_updates(paths: Paths) -> None:
    ...
    update_before = str(date.today() - timedelta(days=Globals.CONFIG["update-window"]))
    for name, repo in specs.names_and_paths[150:]:
        if lock.get_last_check(name) < update_before:

            # hash_, recency = Utils.get_commit_info(repo)
            # print(f"{name:<30}, {recency}")
...
def update_and_install_plugins(args) -> None:
    print("Not yet implemented!")
...
def parse_args() -> argparse.Namespace:
    ...
    all_ = subparsers.add_parser("all", parents=[parent_parser]).add_subparsers(dest="subsubcommand")
    ...
    # 'plugins' subcommand
    plugins.add_parser("install-new", help="Install plugins from plugins.jsonc", parents=[parent_parser])
