...
path_helpers.resolve_existing = function(path)
	...
	if not vim.fn.exists() then error("function'resolve': path " .. path .. " must already exist.") end return resolved end
path_helpers.resolve_and_mkdir = function(path)
	local resolved = path_helpers.resolve(path)
	if not vim.fn.exists() then vim.fn.mkdir(resolved) end
	return resolved
end
