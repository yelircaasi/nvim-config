...
def ext_glob(path: Path, extension: str) -> list[Path]:
...
def search_plugin_directory(dir: Path, plugin_name: str) -> PluginInfo:
    info = PluginInfo()
    lua_files = ext_glob(HERE, "lua")
    vim_files = ext_glob(HERE, "vim")
    txt_files = ext_glob(HERE, "txt")
    md_files = ext_glob(HERE, "txt")
